...
C = {
 ...
 "進捗トラック": lit(r'\.qi\{[^}]*background:(#[0-9a-fA-F]{3,6})', "prog-track"),  # 進捗4分割の空マス（塗り=実績の青）
}
...
PAIRS = [
 ...
 # ⑤ 完了行
 ("完了", "2L"), ("完了", "予定枠"),
 # ⑥ 進捗4分割（空マス=トラック × 塗り=実績／空マスが乗る行背景）
 ("進捗トラック", "実績"), ("進捗トラック", "2L"), ("進捗トラック", "完了"),
]
...
ACCEPTED = {
 ...
 frozenset(("進捗トラック", "2L")): "空マスは淡枠(box-shadow inset)で4分割が常時可視＝形で判別・色非依存。値の信号は塗り(青)×トラックのΔE59で担保",
 frozenset(("進捗トラック", "完了")): "完了行は進捗100%＝空マス無し（溶ける部分が存在しない）。かつ淡枠で冗長化",
}
