...
with sync_playwright() as p:
    ...
    pg.goto(VIEWER)
    check(pg.inner_text("#openBtn") == "ファイルを開く（D&D可）", "既定は日本語")
    pg.evaluate("d=>window.renderData(d)", DATA); pg.wait_for_timeout(150)

    pg.click("#langBtn"); pg.wait_for_timeout(200)
    check(pg.inner_text("#openBtn") == "Open file (or drop)", "EN: ボタン英語化")
    ...
    check(pg.inner_text("#openBtn") == "Open file (or drop)", "リロード後もENを記憶")
    pg.click("#langBtn"); pg.wait_for_timeout(100)
    check(pg.inner_text("#openBtn") == "ファイルを開く（D&D可）", "日本語へ復帰")
    b.close()
finish(errors)
