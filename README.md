...
2. **「ファイルを開く」** で `wbs.json` を選ぶ（または操作バーへドラッグ&ドロップ）
   ※ 同梱の `wbs.json` は**本ツール自身の v1.1 開発計画（実データ）**です。GitHub の Issue と連動し、Claude Code が保守しています — 開けば「このツールが自分自身を管理している」様子が見られます
   （フォーマットの最小例は下の「データ形式」、きれいなサンプルは `tests/正常_*.json` にあります。`_ai` 等の `_` 始まりキーは任意のメタ情報で、必須ではありません）
