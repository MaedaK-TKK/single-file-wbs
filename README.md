...
- **イナズマ線（進捗線）** — 進行中タスクの遅れ/進みを折れ線で一目に（左へ突出＝遅延）
- **工数・進捗は自動計算**（データに状態を持たせない設計） — 工数＝数量×時間÷8（人日）、進捗＝実績日付から推定
...
- **進捗** = 未着手0% / 完了100% / 進行中 `clamp((本日−実績開始) ÷ 予定期間 × 100, 0, 100)`、親は工数加重平均
- **イナズマ線** = 進行中は進捗位置、完了・未着手は本日線上。本日線より左＝遅延
