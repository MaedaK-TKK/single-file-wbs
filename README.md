...
### ブラウザ上で編集（任意）
テキスト/AI 編集に加え、**「編集」ボタン**で画面から直接編集できます（ONで緑）。各フィールドのインライン編集（日付はピッカー）、リーフの追加（`＋`／プロジェクト行の`＋タスク`）・削除（`✕`）・兄弟内の上下移動（`▲▼`）に対応し、**変更は即 `wbs.json` に自動保存**されます（初回のみ書き込み許可ダイアログ）。ドラッグ&ドロップ・親またぎ移動・自動リナンバーは対象外で、それらは JSON か AI 編集で行います。
...
- **進捗**（内部値・数値表示はしない） = 未着手0% / 完了100% / 進行中 `clamp((本日−実績開始) ÷ 予定期間 × 100, 0, 100)`、親は工数加重平均
- **イナズマ線**（左へ突出＝遅延） = 終了遅延（期限超過・未完了）は予定終了日／着手遅延（未着手で予定開始超過）は予定開始日／進行中（期限内）・完了は本日線上
