...
V = rootvars()
C = {
 "予定帯": V["plan"], "予定罫": V["plan-edge"], "実績": V["actual"],
 "親予定": V["pplan"], "親実績": V["pactual"], "proj": V["proj"],
 "1L": V["lvl0"], "2L": V["lvl1"], "完了": V["done-bg"], "完了文字": V["done-fg"],
 "3L": lit(r'\.lvl2\{background:(#[0-9a-fA-F]{3,6})', "lvl2"),
 "gap": lit(r'\.bar\.gap\{[^}]*background:(#[0-9a-fA-F]{3,6})', "gap-bg"),
 "gap罫": lit(r'\.bar\.gap\{[^}]*dashed (#[0-9a-fA-F]{3,6})', "gap-border"),
...
PAIRS = [
 # ① 同レーンのバー同士＋親子（親=青族の濃淡で区別）
 ("予定帯", "gap"), ("gap", "実績"), ("予定帯", "実績"), ("実績", "over"), ("gap", "over"),
 ("親予定", "親実績"), ("親実績", "実績"), ("親予定", "予定帯"),
 # ② バー × 週末列の地色
 ("予定帯", "土"), ("予定帯", "日"), ("実績", "日"), ("gap", "日"), ("gap", "土"), ("over", "日"),
 # ③ オーバーレイ線 × 横切るバー
 ("イナズマ", "実績"), ("イナズマ", "予定帯"), ("イナズマ", "gap"), ("イナズマ", "over"),
 ("MS既定", "実績"), ("MS既定", "予定帯"), ("MS既定", "over"),
 # ④ 階層の隣接
 ("proj", "1L"), ("1L", "2L"), ("2L", "3L"),
 # ⑤ 完了行
 ("完了", "2L"), ("完了", "予定帯"),
...
ACCEPTED = {
 ...
 frozenset(("親予定", "予定帯")): "親/リーフの予定帯は別の行（親=集約行・bold・字下げ）で区別・帯は罫線で定義",
}
...
chs = []
for nm in ["予定帯", "実績", "gap", "over", "親予定", "親実績", "proj", "1L", "2L", "完了"]:
